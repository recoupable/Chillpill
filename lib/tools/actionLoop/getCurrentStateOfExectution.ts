...
export const getCurrentStateOfExecution = async () => {
  ...
  - Emails goal: 11 (current: ${createPosts.length}) ${
    createPosts.length >= 11 ? "✓" : "✗"
  }

  Total completed goals: ${[createPosts.length >= 11].filter(Boolean).length}
  Remaining goals: ${1 - [createPosts.length >= 11].filter(Boolean).length}
